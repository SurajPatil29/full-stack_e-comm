...
import BannerBoxV2Model from "../models/bannerBoxV2.model.js";
...
export async function createBanner(req, res, next) {
	try {
		const { images, catId, subCatId, thirdsubCatId, prodType } = req.body;

		if (!images) {
			return sendError(res, "Images are required", 422);
		}

		const banner = new BannerBoxV2Model({
			images: images,
			catId: catId,
			subCatId: subCatId,
...
export async function getAllBanners(req, res, next) {
	try {
		const banners = await BannerBoxV2Model.find().sort({ createdAt: -1 });
...
export async function getBanner(req, res, next) {
	try {
		...
		const banner = await BannerBoxV2Model.findById(id);
...
export async function removeBannerImage(req, res, next) {
	try {
		...
		const bannerV2 = await BannerBoxV2Model.findOne({ images: imgUrl });
...
export async function updateBanner(req, res, next) {
	try {
		...
		const { images, catId, subCatId, thirdsubCatId, prodType } = req.body; // images should be an array of URLs
		...
		if (subCatId) updateData.subCatId = subCatId;
		if (thirdsubCatId) updateData.thirdsubCatId = thirdsubCatId;
		if (prodType) updateData.prodType = prodType;

		// Update in DB
		const updatedBanner = await BannerBoxV2Model.findByIdAndUpdate(
			id,
			updateData,
			{
				new: true,
...
export async function deleteBanner(req, res, next) {
	try {
		const banner = await BannerBoxV2Model.findById(req.params.id);
		if (!banner) {
		...
		await BannerBoxV2Model.findByIdAndDelete(req.params.id);
...
export async function deleteMultipleBanners(req, res, next) {
	try {
		...
		for (const id of ids) {
			...
			// Delete main Banner
			await BannerBoxV2Model.findByIdAndDelete(id);
		}

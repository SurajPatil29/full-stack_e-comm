...
import BannerBoxV1Model from "../models/bannerBoxV1.model.js";
...
export async function createBanner(req, res, next) {
	try {
		...
		const banner = new BannerBoxV1Model({
			images: images,
			title: title || "BannerBoxV1",
			price: price,
			catId: catId,
			subCatId: subCatId,
...
export async function getAllBanners(req, res, next) {
	try {
		const banners = await BannerBoxV1Model.find().sort({ createdAt: -1 });
...
export async function getBanner(req, res, next) {
	try {
		...
		const banner = await BannerBoxV1Model.findById(id);
...
export async function removeBannerImage(req, res, next) {
	try {
		...
		const bannerV2 = await BannerBoxV1Model.findOne({ images: imgUrl });
...
export async function updateBanner(req, res, next) {
	try {
		...
		const {
			...
			angle,
			slide,
		} = req.body; // images should be an array of URLs
		...
		if (slide) updateData.slide = slide;

		// Update in DB
		const updatedBanner = await BannerBoxV1Model.findByIdAndUpdate(
			id,
			updateData,
			{
				new: true,
...
export async function deleteBanner(req, res, next) {
	try {
		const banner = await BannerBoxV1Model.findById(req.params.id);
		if (!banner) {
		...
		await BannerBoxV1Model.findByIdAndDelete(req.params.id);
...
export async function deleteMultipleBanners(req, res, next) {
	try {
		...
		for (const id of ids) {
			...
			// Delete main Banner
			await BannerBoxV1Model.findByIdAndDelete(id);
		}

	...
	return (
		<>
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-2 py-2 gap-3">
				<h2 className="text-[16px] sm:text-[18px] font-[600]">Category List</h2>
				...
				<div className="flex flex-col sm:flex-row sm:ml-auto w-full sm:w-auto items-stretch sm:items-center gap-2 sm:gap-3">
					...
					{selected.length > 0 && (
						<Button
							className="btn-sm w-full sm:w-auto !bg-red-600 !text-white gap-2 flex items-center justify-center sm:justify-start"
								...
								{catData
									...
									.map((cat) => (
										<TableRow key={cat._id}>
											...
											<TableCell width={100}>
												<div className="w-[50px] sm:w-[80px] flex items-center gap-4">
													<div className="img w-full rounded-md overflow-hidden group">
														<img
															src={cat.images?.[0] || "/placeholder.png"}
															alt={cat.name}
															className="w-full h-[40px] sm:h-[60px] object-cover group-hover:scale-105 transition-all"
														/>
													</div>
												</div>
											</TableCell>
